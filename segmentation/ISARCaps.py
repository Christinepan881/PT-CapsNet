...
class SelfAttentionRouting(nn.Module):
    def __init__(self,ch_in,n_in,w_out ):
        super().__init__()
        self.mask = nn.Conv2d(ch_in*n_in,ch_in,kernel_size=1,groups=ch_in)
    ...
    def forward(self, x):
        #[b,c,n,w,w]->[b,c,n,1,1]
        w_in = x.shape[-1]
        mask = self.mask(x.reshape(-1,self.ch_in*self.n_in,w_in,w_in))#[b,c,w,w]
        mask_x = mask.reshape(-1,self.ch_in,w_in,w_in).unsqueeze(2) * x #[b,c,n,w,w]
