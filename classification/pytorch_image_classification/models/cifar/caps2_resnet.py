...
class Capsules1_2_1_1(nn.Module):
    ''' [b,c,n,h,w]->[b,c',n',h,w] dim转换用group''' 
    def __init__(self, ch_in, n_in, ch_out, n_out, kernel_size=3, stride=1, padding=0, dilation=1, bias=True):
        ...
        self.conv_vector = nn.Conv2d(ch_out*n_in, ch_out*n_out, kernel_size=1, groups=ch_out, bias=bias) 
        #self._init_weight()
    def forward(self, x):
        ...
        h1,w1 = c_map.shape[-2:]
        c_map = c_map.reshape(-1, self.n_in, self.ch_out, h1, w1).permute(0,2,1,3,4).reshape(-1, self.ch_out*self.n_in, h1, w1) #[b,c'n,h',w']

        n_vote = self.conv_vector(c_map) #[b,c'n',h',w']
        
        return n_vote.reshape(-1, self.ch_out, self.n_out, h1, w1) #[b,c',n',h',w']
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, ch_in,n_in,ch_out,n_out, stride):
        ...
        self.caps1 = Capsules1_2_1_1(ch_in,n_in,ch_out,n_in,
                    kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn1 = nn.BatchNorm3d(ch_out)
        self.caps2 = Capsules1_2_1_1(ch_out,n_in,ch_out,n_out,
        ...
        if ch_in != ch_out or n_in != n_out:
            self.shortcut.add_module(
                'downsample_caps1x1',
                Capsules1_2_1_1(ch_in,n_in,ch_out,n_out,
...
class BottleneckBlock(nn.Module):
    ...
    def __init__(self, ch_in,n_in,ch_out,n_out, stride):
        ...
        self.caps1 = Capsules1_2_1_1(ch_in,n_in,bottleneck_channels,n_in,
                    kernel_size=1, stride=1, padding=0, bias=False)
        self.bn1 = nn.BatchNorm2d(bottleneck_channels)

        # downsample with 3x3 caps
        self.caps2 = Capsules1_2_1_1(bottleneck_channels,n_in,bottleneck_channels,n_out,
                    kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn2 = nn.BatchNorm3d(bottleneck_channels)

        self.caps3 = Capsules1_2_1_1(bottleneck_channels,n_out,ch_out,n_out,
                    kernel_size=1, stride=1, padding=0, bias=False)
...
class Network(nn.Module):
    def __init__(self, config):
        ...
        self.caps = Capsules1_2_1_1(config.dataset.n_channels,1,
                              chs[0], ns[0],
                              kernel_size=3,
                              stride=1,
                              padding=1,
        ...
        #        dtype=torch.float32)
        #    fe_w = self._forward_caps(dummy_data).shape[-1]
            #[b,c,n,h,w]
            #view(-1).shape[0]
        self.shrink = None
        if config.dataset.image_size == 32:
            self.shrink = nn.Sequential(
                Capsules1_2_1_1(16, 8, 8, 4, kernel_size=4, stride=4, bias=False), #2
                Capsules1_2_1_1(8, 4, 16, 8, kernel_size=2, stride=2, bias=False), #1 #[b,c,n,1,1]
                Capsules1_2_1_1(16, 8, config.dataset.n_classes, 8,
                    kernel_size=1, bias=False), #fc1
                Capsules1_2_1_1(config.dataset.n_classes,8,config.dataset.n_classes,1,
                    kernel_size=1, bias=False), #fc2 #[b,num,1,1,1]
            ) 
        else:
